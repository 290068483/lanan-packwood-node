  ...
  try {
    ...
    if (!fs.existsSync(localPackagePath)) {
      ...
      logSuccess(
        customerName,
        'PACKAGE_CHECK',
        `已创建package.json文件: ${localPackagePath}`
      );
    }
...
async function syncPackageAndData(cabinets, outputDir, customerName) {
  try {
    ...
    if (dataChanged && !packageChanged) {
      ...
      console.log(`✓ 客户 "${customerName}" package.json已同步更新`);
      logSuccess(customerName, 'PACKAGE_SYNC', 'package.json已同步更新');
    }
    ...
    const errorMsg = `同步package和数据变化检查时发生错误: ${error.message}`;
    console.error(`✗ ${errorMsg}`);
    logError(customerName, 'SYNC_CHECK', errorMsg, error.stack);
    // 出错时默认都已变化
 ...
 * @param {string} customerName - 客户名称
 */
async function createSimplifiedXmlDirectly(
  xmlFilePath,
  outputDir,
  customerName
) {
  try {
    ...
    simplifiedXml = simplifiedXml.replace(
      /<Metals\b[^>]*>[\s\S]*?<\/Metals>/g,
      ''
    );

    // 删除Hardwares节点（包括内容）
    simplifiedXml = simplifiedXml.replace(
      /<Hardwares\b[^>]*>[\s\S]*?<\/Hardwares>/g,
      ''
    );

    // 删除ExtendBomLists节点（包括内容）
    simplifiedXml = simplifiedXml.replace(
      /<ExtendBomLists\b[^>]*>[\s\S]*?<\/ExtendBomLists>/g,
      ''
    );
...
function parseXmlInSegments(xmlData, lineDir, customerName) {
  ...
  try {
    // 尝试找到所有Cabinet节点
    const cabinetMatches = xmlData.match(
      /<Cabinet\s+[^>]*>[\s\S]*?<\/Cabinet>/g
    );
    if (!cabinetMatches) {
    ...
    logInfo(
      customerName,
      lineDir,
      `找到 ${cabinetMatches.length} 个Cabinet节点`
    );
    ...
    for (let i = 0; i < cabinetMatches.length; i++) {
      try {
        ...
        logWarning(
          customerName,
          lineDir,
          `解析第 ${i + 1} 个Cabinet节点失败: ${segmentError.message}`
        );
        console.warn(
          `  解析第 ${i + 1} 个Cabinet节点失败: ${segmentError.message}`
        );
    ...
    logInfo(
      customerName,
      lineDir,
      `分段解析完成: 成功 ${successCount}, 失败 ${failCount}`
    );
 ...
 * @param {string} customerName - 客户名称
 */
function extractPanelsAndPanelsDirectly(xmlData, lineDir, customerName) {
  ...
  try {
    ...
    for (let i = 0; i < panelMatches.length; i++) {
      try {
        ...
        logWarning(
          customerName,
          lineDir,
          `解析第 ${i + 1} 个Panel节点失败: ${panelError.message}`
        );
        console.warn(
          `  解析第 ${i + 1} 个Panel节点失败: ${panelError.message}`
        );
    ...
    logInfo(
      customerName,
      lineDir,
      `Panel节点提取完成: 成功 ${successCount}, 失败 ${failCount}`
    );
 ...
 */
function getErrorPosition(xmlData, error) {
  try {
    // 尝试从错误消息中提取行号信息
...
function parseXmlWithFallback(xmlData, lineDir, customerName) {
  ...
  try {
    ...
    logWarning(
      customerName,
      lineDir,
      `fast-xml-parser最宽松配置解析失败: ${error.message}`,
      errorContext
    );
    ...
    try {
      ...
      logWarning(
        customerName,
        lineDir,
        `xml2js解析器失败: ${xml2jsError.message}`,
        xml2jsErrorContext
      );
      ...
      try {
        ...
        logWarning(
          customerName,
          lineDir,
          `xmldom解析器失败: ${xmldomError.message}`,
          xmldomErrorContext
        );
        ...
        try {
          ...
          logWarning(
            customerName,
            lineDir,
            `libxmljs2解析器失败: ${libxmljsError.message}`,
            libxmljsErrorContext
          );
          ...
          try {
            ...
            try {
              logInfo(
                customerName,
                lineDir,
                '尝试使用修复后的XML数据和fast-xml-parser最宽松配置再次解析'
              );
              console.log(
                '  尝试使用修复后的XML数据和fast-xml-parser最宽松配置再次解析'
              );
              ...
              logWarning(
                customerName,
                lineDir,
                `修复后fast-xml-parser最宽松配置解析仍失败: ${repairedError.message}`,
                repairedErrorContext
              );
              console.warn(
                `  修复后fast-xml-parser最宽松配置解析仍失败: ${repairedError.message}`
              );
              ...
              try {
                ...
                logWarning(
                  customerName,
                  lineDir,
                  `修复后xml2js解析仍失败: ${repairedXml2jsError.message}`,
                  repairedXml2jsErrorContext
                );
                console.warn(
                  `  修复后xml2js解析仍失败: ${repairedXml2jsError.message}`
                );
                ...
                try {
                  ...
                  logWarning(
                    customerName,
                    lineDir,
                    `修复后xmldom解析仍失败: ${repairedXmldomError.message}`,
                    repairedXmldomErrorContext
                  );
                  console.warn(
                    `  修复后xmldom解析仍失败: ${repairedXmldomError.message}`
                  );
                  ...
                  try {
                    ...
                    logWarning(
                      customerName,
                      lineDir,
                      `修复后libxmljs2解析仍失败: ${repairedLibxmljsError.message}`,
                      repairedLibxmljsErrorContext
                    );
                    console.warn(
                      `  修复后libxmljs2解析仍失败: ${repairedLibxmljsError.message}`
                    );
                    ...
                    try {
                      ...
                      const segmentResult = parseXmlInSegments(
                        xmlData,
                        lineDir,
                        customerName
                      );
                      ...
                      try {
                        logInfo(
                          customerName,
                          lineDir,
                          '尝试直接查找Panels和Panel节点'
                        );
                        console.log('  尝试直接查找Panels和Panel节点');
                        const panelResult = extractPanelsAndPanelsDirectly(
                          xmlData,
                          lineDir,
                          customerName
                        );
                        ...
                        logWarning(
                          customerName,
                          lineDir,
                          `直接查找Panels和Panel节点也失败: ${panelError.message}`,
                          panelErrorContext
                        );
                        console.warn(
                          `  直接查找Panels和Panel节点也失败: ${panelError.message}`
 ...
 */
function extractPanelsFromParsedData(
  parsedData,
  xmlData,
  customerName,
  lineDir
  ...
  try {
    ...
    if (parsedData && parsedData.Root && parsedData.Root.Cabinet) {
      logInfo(
        customerName,
        lineDir,
        `解析后数据结构关键字段: ${Object.keys(parsedData.Root)}`
      );
    ...
    if (cabinets.length === 0) {
      ...
      if (panelMatches && panelMatches.length > 0) {
        console.log(
          `ℹ ${customerName} 从XML文本中找到 ${panelMatches.length} 个Panel节点`
        );
        logInfo(
          customerName,
          lineDir,
          `从XML文本中找到 ${panelMatches.length} 个Panel节点`
        );
        ...
        for (let i = 0; i < Math.min(panelMatches.length, 20); i++) {
          // 限制解析前20个以避免过多日志
          try {
            const panelXml = `<Root>${panelMatches[i]}</Root>`;
        ...
        console.log(
          `ℹ ${customerName} 成功解析 ${successfullyParsed} 个Panel节点`
        );
        logInfo(
          customerName,
          lineDir,
          `成功解析 ${successfullyParsed} 个Panel节点`
        );
...
async function autoDetectAndProcessLines() {
  try {
    ...
    console.log(
      `✓ [SYSTEM] 检测到 ${customerDirs.length} 个客户目录: ${customerDirs.join(', ')}`
    );
    ...
    for (const customerDir of customerDirs) {
      ...
      try {
        ...
        // 检查package.json是否发生变化
        const isPackageChanged = await checkPackageChanged(
          customerOutputDir,
          customerDir
        );
    ...
    console.error(
      `✗ [SYSTEM] 自动检测并处理产线数据时发生错误: ${error.message}`
    );
    return false;
  }
}
...
async function processCustomerData(
  ...
  try {
    ...
    if (!fs.existsSync(customerDevicePath)) {
      console.log(
        `⚠ 客户 "${customerName}" 的设备文件路径不存在: ${customerDevicePath}`
      );
      logWarning(
        customerName,
        'MAIN',
        `设备文件路径不存在: ${customerDevicePath}`
      );
      return false;
    }

    // 获取所有产线目录
    const productionLineDirs = fs
      .readdirSync(customerDevicePath)
      .filter(file =>
    ...
    console.log(
      `✓ 检测到 ${productionLineDirs.length} 个产线目录:`,
      productionLineDirs
    );
    logSuccess(
      customerName,
      'MAIN',
      `检测到 ${productionLineDirs.length} 个产线目录: ${productionLineDirs.join(', ')}`
    );
    ...
    for (const lineDir of productionLineDirs) {
      ...
      try {
        ...
        logSuccess(
          customerName,
          'XML_READ',
          `成功读取XML文件，大小: ${xmlData.length} 字符`
        );
        ...
        for (const parserConfig of parserConfigs) {
          try {
            console.log(
              `ℹ [${customerName}] 尝试使用fast-xml-parser${parserConfig.name === 'standard' ? '标准' : parserConfig.name === 'loose' ? '宽松' : '最宽松'}解析器解析XML`
            );
            logInfo(
              customerName,
              'XML_PARSE',
              `尝试使用fast-xml-parser${parserConfig.name === 'standard' ? '标准' : parserConfig.name === 'loose' ? '宽松' : '最宽松'}解析器解析XML`
            );
            ...
            if (parsedData && Object.keys(parsedData).length > 0) {
              console.log(
                `ℹ [${customerName}] 解析后数据结构关键字段: ${Object.keys(parsedData).join(',')}`
              );
              logInfo(
                customerName,
                'XML_PARSE',
                `解析后数据结构关键字段: ${Object.keys(parsedData).join(',')}`
              );
              break;
            }
          } catch (parseError) {
            console.log(
              `⚠ [${customerName}] fast-xml-parser${parserConfig.name === 'standard' ? '标准' : parserConfig.name === 'loose' ? '宽松' : '最宽松'}解析器失败: ${parseError.message}`
            );
            logWarning(
              customerName,
              'XML_PARSE',
              `fast-xml-parser${parserConfig.name === 'standard' ? '标准' : parserConfig.name === 'loose' ? '宽松' : '最宽松'}解析器失败: ${parseError.message}`
            );
          }
        }
        ...
        if (!parsedData || Object.keys(parsedData).length === 0) {
          try {
            ...
            console.log(
              `⚠ [${customerName}] xml2js解析器也失败: ${xml2jsError.message}`
            );
            logWarning(
              customerName,
              'XML_PARSE',
              `xml2js解析器也失败: ${xml2jsError.message}`
            );
          }
        }
        ...
        if (!parsedData || Object.keys(parsedData).length === 0) {
          try {
            console.log(
              `ℹ [${customerName}] 尝试直接从XML文本中提取Panel节点`
            );
            logInfo(
              customerName,
              'XML_PARSE',
              '尝试直接从XML文本中提取Panel节点'
            );

            // 使用正则表达式匹配Panel节点
            const panelMatches = xmlData.match(
              /<Panel\s+[^>]*>[\s\S]*?<\/Panel>/g
            );
            if (panelMatches && panelMatches.length > 0) {
              console.log(
              ...
              console.log(
                `ℹ ${customerName} 成功解析 ${panels.length} 个Panel节点`
              );
              logInfo(
                customerName,
                'XML_PARSE',
                `成功解析 ${panels.length} 个Panel节点`
              );
            ...
            console.log(
              `⚠ [${customerName}] 直接从XML文本提取Panel节点也失败: ${directParseError.message}`
            );
            logWarning(
              customerName,
              'XML_PARSE',
              `直接从XML文本提取Panel节点也失败: ${directParseError.message}`
            );
          }
        }
        ...
        cabinets = cabinets.filter(
          cabinet => cabinet && Object.keys(cabinet).length > 0
        );
        ...
        if (cabinets.length > 0) {
          ...
          logWarning(
            customerName,
            'XML_PROCESSING',
            `产线 "${lineDir}" 未找到有效的Cabinet节点`
          );
        }
      } catch (error) {
        console.error(
          `✗ 处理产线 "${lineDir}" 的XML文件时出错:`,
          error.message
        );
        logError(
          customerName,
          'XML_PROCESSING',
          `处理产线 "${lineDir}" 的XML文件时出错: ${error.message}`,
          error.stack
        );
        // 不立即退出，继续处理其他产线
      }
    ...
    // 检查数据是否发生变化
    const isDataChanged = await checkDataChanged(
      allCabinets,
      customerOutputDir,
      customerName
    );
    ...
    try {
      ...
      const firstXmlFilePath = path.join(
        customerDevicePath,
        firstLineDir,
        '0、排版文件',
        '优化文件.xml'
      ...
      if (fs.existsSync(firstXmlFilePath)) {
        ...
        await createSimplifiedXmlDirectly(
          firstXmlFilePath,
          customerOutputDir,
          customerName
        );
...
async function generateSimplifiedXml(customerData, outputDir, customerName) {
  try {
    ...
    if (simplifiedData && simplifiedData.Root) {
      ...
      if (simplifiedData.Root.Cabinet) {
        ...
        if (Array.isArray(simplifiedData.Root.Cabinet)) {
          simplifiedData.Root.Cabinet = simplifiedData.Root.Cabinet.map(
            cabinet => {
              // 检查 Cabinet 是否有效
          ...
          // 单个 Cabinet 对象
          if (
            simplifiedData.Root.Cabinet &&
            typeof simplifiedData.Root.Cabinet === 'object'
          ) {
    ...
    try {
      ...
      console.warn(
        `⚠ XMLBuilder构建失败，尝试使用xml2js: ${buildError.message}`
      );
      logWarning(
        customerName,
        'XML_GENERATION',
...
async function generateExcel(
  ...
  try {
    ...
    cabinets.forEach((cabinet, cabinetIndex) => {
      ...
      panels.forEach((item, panelIndex) => {
        ...
        const dataRow = worksheet.addRow([
          ...
          item['@_HasHorizontalHole'] === 1
            ? '是'
            : item['@_HasHorizontalHole'] === 0
              ? '否'
              : '',
          '', // 槽铣
          '', // 拉直
    ...
    const outputFileName = path.join(
      outputDir,
      config.targetFileName.replace('.xlsx', `_${timestamp}.xlsx`)
    );
    ...
    logSuccess(
      customerName,
      'EXCEL_GENERATION',
      `Excel表格已生成到 ${outputFileName}，共处理 ${totalPanels} 行数据`
    );
...
async function syncToNetwork(outputDir, customerName, packagedRows, totalRows) {
  ...
  try {
    ...
    if (fs.existsSync(customerLocalPath)) {
      sourceExcelFileName = fs
        .readdirSync(customerLocalPath)
...
if (require.main === module) {
  async function main() {
    ...
    logSuccess(
      'SYSTEM',
      'MAIN',
      `检测到 ${customers.length} 个客户目录: ${customers.join(', ')}`
    );
