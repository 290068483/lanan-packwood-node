...
console.log('启动热更新开发环境...');

// 启动热更新服务器
const hotServer = spawn('node', [path.join(__dirname, 'hot-reload-server.js')]);
...
setTimeout(() => {
  ...
  const electronApp = spawn('electron', [path.join(__dirname, 'index-with-hot.html'), '--hot'], {
    stdio: 'inherit'
  });
