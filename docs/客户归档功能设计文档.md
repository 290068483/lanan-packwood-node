...
创建测试环境配置文件 `tests/config.test.json`：
...
#### 7.2.1 CustomerArchiveManager 类测试

创建 `tests/customer-archive-manager.test.js` 文件，测试归档管理模块的主要功能：
...
beforeAll(async () => {
  ...
  // 创建测试输出目录
  await fs.mkdir(mockCustomerData.outputPath, { recursive: true });
  
  // 创建模拟的packages.json文件
  const packagesPath = path.join(mockCustomerData.outputPath, 'packages.json');
  await fs.writeFile(packagesPath, JSON.stringify(mockPackagesData));
  
  // 模拟数据库连接
  Database.getInstance = jest.fn().mockReturnValue({
    getConnection: jest.fn().mockResolvedValue({
      ...
      query: jest.fn()
        ...
        .mockResolvedValueOnce([{ insertId: 1 }]) // 归档列表查询
        .mockResolvedValueOnce([{ insertId: 1 }]) // 归档详情查询
    }),
    query: jest.fn()
      .mockResolvedValueOnce([mockCustomerData]) // 获取客户数据
      .mockResolvedValueOnce([{ total: 10 }]) // 归档总数查询
  });
  
  // 模拟DataManager.getCustomer
  DataManager.getCustomer = jest.fn().mockResolvedValue(mockCustomerData);
  
  // 模拟DataManager.updateCustomerStatus
  ...
  fs.existsSync = jest.fn()
    .mockReturnValue(true) // packages.json存在
    .mockReturnValueOnce(true) // 备份文件存在
    .mockReturnValueOnce(false); // 备份文件不存在
});

afterAll(async () => {
  // 清理测试目录
  try {
    await fs.rm(testConfig.outputPath, { recursive: true, force: true });
  } catch (error) {
...
describe('CustomerArchiveManager', () => {
  describe('archiveCustomer', () => {
    it('应成功归档客户数据', async () => {
      ...
      expect(fs.rm).toHaveBeenCalledWith(mockCustomerData.outputPath, { recursive: true, force: true });
    ...
    it('应处理归档过程中的错误', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.message).toContain('删除目录失败');
    });
  });
  ...
  describe('getArchiveDetail', () => {
    ...
    it('应处理不存在的归档ID', async () => {
      // 模拟数据库返回空结果
      Database.getInstance = jest.fn().mockReturnValue({
        query: jest.fn().mockResolvedValueOnce([[]]) // 归档不存在
      });
  ...
  describe('restoreArchive', () => {
    ...
    it('应处理不存在的归档', async () => {
      // 模拟不存在的归档
      CustomerArchiveManager.getArchiveDetail = jest.fn().mockResolvedValue({
        success: false,
        message: '归档记录不存在'
      });
...
创建 `tests/database-archive.test.js` 文件，测试数据库操作：
...
describe('数据库归档操作', () => {
  ...
  beforeAll(async () => {
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS package_archive (
        id INT PRIMARY KEY AUTO_INCREMENT,
        archive_id INT NOT NULL,
        pack_seq VARCHAR(50) NOT NULL,
        package_weight DECIMAL(10,2),
  ...
  afterAll(async () => {
    ...
    await connection.query('DROP TABLE IF EXISTS part_archive');
    await connection.query('DROP TABLE IF EXISTS package_archive');
    await connection.query('DROP TABLE IF EXISTS customer_archive');
  ...
  describe('归档数据存储', () => {
    ...
    it('应成功查询归档数据', async () => {
      ...
      expect(archives[0].customer_name).toBe('测试客户');
      ...
      expect(packages[0].pack_seq).toBe('包1');
      ...
      expect(parts[0].part_id).toBe('ID001');
    ...
    it('应测试数据完整性约束', async () => {
      ...
      try {
        await connection.query(
          `INSERT INTO package_archive (archive_id, pack_seq, created_at) 
          VALUES (999, '测试包', NOW())`
        );
        
        // 如果执行到这里，说明测试失败，因为没有抛出错误
  ...
  describe('事务测试', () => {
    it('应成功提交事务', async () => {
      ...
      try {
        ...
        const [archiveResult] = await connection.query(
          `INSERT INTO customer_archive 
          (customer_name, archive_date, backup_path, packages_count, total_parts_count, archive_user) 
          VALUES (?, NOW(), ?, ?, ?, ?)`,
          ['事务测试客户', 'C:\\backup\\test2.zip', 1, 3, '测试操作员']
        );
        ...
        await connection.query(
          `INSERT INTO package_archive 
          (archive_id, pack_seq, created_at) 
          VALUES (?, ?, NOW())`,
          [archiveId, '事务测试包']
        );
        ...
        // 验证数据是否插入成功
        const [archives] = await connection.query(
          `SELECT * FROM customer_archive WHERE customer_name = ?`,
          ['事务测试客户']
        );
        
        expect(archives.length).toBe(1);
        expect(archives[0].customer_name).toBe('事务测试客户');
    ...
    it('应正确回滚事务', async () => {
      ...
      try {
        ...
        const [archiveResult] = await connection.query(
          ...
          (customer_name, archive_date, backup_path, packages_count, total_parts_count, archive_user) 
          VALUES (?, NOW(), ?, ?, ?, ?)`,
          ['回滚测试客户', 'C:\\backup\\test3.zip', 1, 3, '测试操作员']
        );
...
- 归档管理模块的测试覆盖率应达到 90% 以上
- 数据库操作的测试覆盖率应达到 85% 以上
- 关键错误处理路径必须有对应的测试用例
...
将单元测试集成到 CI/CD 流程中，确保每次代码提交都会自动运行测试：
