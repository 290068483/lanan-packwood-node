...
基于 packages.json 中的 partIDs 数据，设计以下客户状态：

1. **未打包**：客户数据已导入，但在 packages.json 中没有对应的 partIDs 记录
2. **正在处理**：客户数据正在打包过程中，部分 partIDs 可能已记录在 packages.json 中
3. **已打包**：客户所有数据都已在 packages.json 中找到对应的 partIDs 记录
4. **已归档**：已打包的客户数据已归档保存 -新增已归档 按钮的处理逻辑
   a.把当前对应的客户文件夹的所有数据备份到一个地方（需要压缩）
   b.同时保存该用户的信息到数据库中，（包括客户的 id，姓名，打的包的信息，以及每个包的 partIDs）
5. **已出货**：已归档的客户货物已出货
6. **未出货**：已归档的客户货物尚未出货

### 2. 状态判断逻辑

- **未打包**：客户数据在 packages.json 中找不到对应的 partIDs 记录
- **正在处理**：部分 partIDs 已在 packages.json 中找到，但不是全部
- **已打包**：客户所有 partIDs 都在 packages.json 中找到对应记录
...
function checkPackStatus(customerData, packagesData) {
  ...
  if (packagesData && Array.isArray(packagesData)) {
    packagesData.forEach(packageItem => {
      if (packageItem.partIDs && Array.isArray(packageItem.partIDs)) {
        // 检查这个包是否包含客户的板件
        const hasCustomerParts = packageItem.partIDs.some(partID =>
...
- 定期检查 packages.json 文件的变化
- 当 packages.json 更新时，重新计算客户状态
...
- 当客户所有板件都找到对应的 partIDs 时，状态自动更新为"已打包"
- 当部分板件找到对应的 partIDs 时，状态为"正在处理"
- 当没有板件找到对应的 partIDs 时，状态为"未打包"
...
5. **文件监控集成**
   - 集成 packages.json 文件监控
   - 实现自动状态更新

### 2. 实施优先级

1. **高优先级**：数据结构扩展、状态判断算法实现、界面更新
2. **中优先级**：状态变更 API 实现、文件监控集成
...
- **主要风险**：packages.json 格式可能存在变化
- **应对措施**：实现健壮的数据解析逻辑，处理各种可能的格式变化
- **备选方案**：如果 packages.json 格式变化太大，可以引入配置文件定义解析规则
...
old 系统中的打包状态判断主要基于以下逻辑：

- 通过遍历 packages.json 文件中的 partIDs 数组
- 将 partIDs 与客户板件明细中的 ID 进行匹配
- 如果所有板件 ID 都能在 packages.json 中找到对应记录，则认为客户已打包完成

### 2. old 系统中的打包记录生成

old 系统中的打包记录生成过程：

1. 读取 packages.json 文件，获取所有包信息
2. 遍历每个包中的 partIDs
3. 根据 partIDs 在原始板件明细中查找对应记录
4. 将匹配的板件信息记录到"已包板件"表格中
5. 标记未找到的板件（显示为"<没有此板排版记录>"）

### 3. old 系统中的打包状态检查

在 old 系统中，通过"遍历列表记录文件各和 json 时间"函数实现：

- 检查 local 文件夹中各子文件夹的 json 文件时间
- 根据 json 文件的保存时间刷新打包记录
- 比较 json 文件时间与列表记录时间，判断是否有更新
